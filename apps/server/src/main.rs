...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/servers/{id}/channels", get(server_core::list_channels))
        .route("/api/servers/{id}/channels", post(server_core::create_channel))
        ...
        .route("/api/servers/{id}/members/rank", post(server_core::set_rank))
        .route("/api/channels/{id}/messages", get(chat_handler::list_messages))
        .route("/api/channels/{id}/messages", post(chat_handler::send_message))
        ...
        .route("/api/dms/{dm_id}/messages", get(chat_handler::list_dm_messages))
        .route("/api/dms/{dm_id}/messages", post(chat_handler::send_dm_message))
...
async fn bind_with_prompt(addr: SocketAddr) -> anyhow::Result<tokio::net::TcpListener> {
    match tokio::net::TcpListener::bind(addr).await {
        ...
        Err(err) if port_guard::is_addr_in_use(&err) => {
            let freed = tokio::task::spawn_blocking(move || port_guard::ask_terminate_and_free(addr))
                .await??;
            if !freed {
                anyhow::bail!(
